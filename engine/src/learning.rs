...
impl LearningEngine {
    ...
    pub fn penalize_usage(mind: &mut OmniMind, key: &str) {
        ...
        match &mut mind.state {
            ...
            crate::LifecycleState::Runtime(_, delta) => {
                if let Some(entry) = delta.metadata.get_mut(key) {
                    entry.confidence *= 0.8;
                    entry.importance *= 0.9;
                }
